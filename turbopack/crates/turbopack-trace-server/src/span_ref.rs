...
use crate::{
    ...
    span::{
        Span, SpanEvent, SpanExtra, SpanGraphEvent, SpanIndex, SpanNames, SpanTimeData, SpanTotals,
    },
...
pub type GroupNameToDirectAndRecusiveSpans =
    FxIndexMap<(RcStr, RcStr), (Vec<SpanIndex>, Vec<SpanIndex>)>;
...
impl<'a> SpanRef<'a> {
    ...
    pub fn group_name(&self) -> (RcStr, RcStr) {
    ...
    pub fn args(&self) -> impl Iterator<Item = (&'a RcStr, &'a RcStr)> {
    ...
    fn search_index(&self) -> &HashMap<RcStr, Vec<SpanIndex>> {
        self.extra().search_index.get_or_init(|| {
            ...
            fn add_span_to_map<'a>(index: &mut HashMap<RcStr, Vec<SpanIndex>>, span: SpanRef<'a>) {
                ...
                if !cat.is_empty() {
                    push_to_index(index, cat.as_str(), || cat.clone(), span.index());
                }
                if !name.is_empty() {
                    push_to_index(
                        index,
